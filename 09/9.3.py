...
json_string = stringify(alphabet)
print(json_string)


# from json import load as download_and_parse
...
#     year: int
#     price: int
# class ManagerSales(TypedDict):
#     manager: Manager
#     cars: list[Car]
# with open("./manager_sales.json", encoding="utf-8") as file:
#     managers_sales: list[ManagerSales] = download_and_parse(file)
#     print(
#         *max(
#             (
#                 {
#                     "first_name": manager_sales["manager"]["first_name"],
#                     "last_name": manager_sales["manager"]["last_name"],
#                     "sales_amount": sum(
#                         (car["price"] for car in manager_sales["cars"])
#                     ),
#                 }
#                 for manager_sales in managers_sales
#             ),
...
#     groups_peoples: list[Group] = download_and_parse(file)
#     print(
#         *max(
#             (
#                 {
#                     "id group": group_peoples["id_group"],
#                     "female born after 1977": len(
#                         [
#                             people
#                             for people in group_peoples["people"]
#                             if people["gender"] == "Female" and people["year"] > 1977
#                         ]
#                     ),
#                 }
#                 for group_peoples in groups_peoples
#             ),
#             key=lambda item: item["female born after 1977"],
#         ).values(),
#         sep=" ",
#     )
...
#     alphabet: dict[str, str] = download_and_parse(file)
# with open("./Abracadabra__1_.txt", encoding="utf-8") as file:
#     print("".join((alphabet.get(char, char) for line in file for char in line)))
...
class People(TypedDict):
    ...
    country: str
    age: int
peoples: list[People] = parse(people)
for people in sorted(peoples, key=lambda item: (item["age"], item["name"])):
...
"""
print(parse(json_string)["customers"][0]["username"])
